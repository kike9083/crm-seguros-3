...
const LeadsList: React.FC = () => {
    ...
    const filteredLeads = useMemo(() => {
        return leads.filter(lead => {
            // 1. Filtro de texto (Búsqueda general en campos nuevos tambien)
            ...
            const matchesSearch = !searchTerm || (
                ...
                (lead.cedula?.toLowerCase() || '').includes(lowerTerm) ||
                (lead.empresa?.toLowerCase() || '').includes(lowerTerm) ||
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div className="flex flex-col gap-4 w-full md:w-auto flex-grow">
                    ...
                    <div className="relative w-full md:max-w-md">
                        <input
                            type="text"
                            placeholder="Buscar por nombre, cédula, empresa, email..."
                            value={searchTerm}
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                        ...
                        >
                            ...
                            {profiles.filter(p => p.rol === 'AGENTE' || p.rol === 'ADMIN').map(agent => (
                ...
                <div className="flex space-x-2">
                    <button onClick={() => setIsImportModalOpen(true)} className="flex items-center bg-secondary hover:bg-gray-600 text-white font-bold py-2 px-4 rounded-lg transition-colors whitespace-nowrap border border-border">
                        <CloudArrowUpIcon className="w-5 h-5 mr-2"/> Importar
                    </button>
                    <button onClick={() => handleOpenModal(null)} className="flex items-center bg-primary hover:bg-accent text-white font-bold py-2 px-4 rounded-lg transition-colors whitespace-nowrap">
            ...
            <div className="bg-card p-6 rounded-lg shadow-lg">
                <div className="overflow-x-auto">
                        ...
                        <tbody>
                            {filteredLeads.map(lead => {
                                ...
                                <tr key={lead.id} className="border-b border-border hover:bg-secondary">
                                    ...
                                    <td className="p-4 text-text-secondary text-sm">{lead.cedula || '-'}</td>
                                    <td className="p-4 text-text-secondary text-sm">{lead.empresa || '-'}</td>
                                    <td className="p-4 text-text-secondary text-sm">{lead.email}</td>
                                    <td className="p-4 text-text-secondary">
                                        {lead.telefono1 ? (
                                            <a href={`tel:${lead.telefono1}`} className="hover:text-white hover:underline transition-colors">{lead.telefono1}</a>
                                        ) : ''}
                                    </td>
                                    <td className="p-4">
                                        {rawPhone ? (
                                    ...
                                    <td className="p-4 text-blue-300 text-sm">{lead.agent_id ? agentMap.get(lead.agent_id) : 'N/A'}</td>
                                    <td className="p-4 space-x-2 whitespace-nowrap text-sm">
                    ...
                    {filteredLeads.length === 0 && (
                        <p className="text-center p-8 text-text-secondary">No se encontraron leads.</p>
                    )}
                </div>
            </div>
            {isModalOpen && <LeadModal lead={selectedLead} onClose={handleCloseModal} onSave={handleSave} />}
            {isImportModalOpen && <ImportModal onClose={() => setIsImportModalOpen(false)} onSuccess={handleSave} />}
            <ConfirmationModal isOpen={isConfirmModalOpen} title="Confirmar Eliminación" message={`¿Borrar "${leadToDelete?.nombre}"?`} onConfirm={handleConfirmDelete} onCancel={handleCancelDelete} confirmText="Eliminar" />
        </>
    );
};
