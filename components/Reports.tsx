...
import { getExpiringPolicies, getErrorMessage, getAllProfiles } from '../services/api';
...
const Reports: React.FC = () => {
    ...
    useEffect(() => {
        const fetchPoliciesAndProfiles = async () => {
            try {
                setLoading(true);
                setError(null);
                ...
                setError(`No se pudieron cargar las pólizas por vencer o perfiles: ${getErrorMessage(err)}`);
                console.error(err);
            } finally {
                setLoading(false);
            }
